...
# Load the datasets
file1 = "excel_model_train\general_and_medical_information.xlsx"
file2 = "excel_model_train\general_and_medical_information.xlsx"
...
model1 = cb.CatBoostClassifier(iterations=500, depth=6, learning_rate=0.1, loss_function='Logloss', verbose=0)
...
model2 = cb.CatBoostClassifier(iterations=500, depth=6, learning_rate=0.1, loss_function='Logloss', verbose=0)
model2.fit(X2_train, y2_train)
preds2 = model2.predict(X2_test)
print(f"Model 2 Accuracy: {accuracy_score(y2_test, preds2):.4f}")

# Save Models
with open("catboost_general_information.pkl", "wb") as f:
    pickle.dump(model1, f, protocol=pickle.HIGHEST_PROTOCOL)
with open("catboost_general_and_medical_information.pkl", "wb") as f:
    pickle.dump(model2, f, protocol=pickle.HIGHEST_PROTOCOL)

# Save using joblib (recommended for large models)
joblib.dump(model1, "catboost_general_information.joblib")
joblib.dump(model2, "catboost_general_and_medical_information.joblib")
